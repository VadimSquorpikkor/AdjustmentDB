...
//        data.put(UNIT_DESCRIPTION, unit.getDescription());//todo УБРАТЬ!!!!
...
//        data.put(UNIT_LOCATION, unit.getLocation());//todo УБРАТЬ!!!!
        data.put(UNIT_SERIAL, unit.getSerial());
        data.put(UNIT_STATE, unit.getState());//todo УБРАТЬ!!!!
...
//                .update(data)
    ...
    void closeEvent(String event_id) {
        Log.e(TAG, "*********closeEvent: "+ event_id);
        if (event_id==null) return;
//        Map<String, Object> data = new HashMap<>();
//        data.put(EVENT_CLOSE_DATE, new Date());
        db.collection(TABLE_EVENTS).document(event_id).update(EVENT_CLOSE_DATE, new Date());
//        db.collection(TABLE_EVENTS).document(event_id).set(data);
    }
