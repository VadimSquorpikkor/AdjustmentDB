...
* <b>1.97 - 14/02/22</b>
1. Исправлено: при выборе в настройках "Автостарт ввода параметров", часто диалог открывался с незагруженными значениями (не успевал получить данные из БД). Надо тестировать
* <b>1.96 - 14/02/22</b>
1. Добавлено: при сохранении юнита теперь в БД (в units) сохраняется дата последнего события (добавлено поле last_events_date)
* <b>1.95 - 08/02/22</b>
1. Добавлено распознавание БДПБ, БДПА, БДПС, БДРМ, БОИ
* <b>1.94 - 07/02/22</b>
1. Исправлено: периодически при старте приложения не находился пользователь, приходилось перезапускать. По-факту: при проверке пользователя в checkUser если пользователь найден в БД, то он сразу же добавляется в список пользователей, после чего уже запускается locationListener
* <b>1.93 - 06/02/22</b>
1. Упрощен FirebaseHelper (методы locationListener, stateListener, deviceListener, devSetListener), доделан Casher
* <b>1.92 - 04/02/22</b>
1. В Casher добавлены методы для сохранения статусов, дополнены соответственно методы в FireDBHelper. Уменьшилось количество обращений к БД
2. Окончена денормализация БД (names, name_id и id пока оставил для обратной совместимости)
* <b>1.91 - 04/02/22</b>
1. Глобально: начало денормализации БД, отказ от таблицы names (и join). Для location, device_set, employees, devices переделаны методы для использования без полей id и name_id. Id теперь это имя документа; вместо ссылки на таблицу имен теперь добавлены поля name_ru, name_en, поля id и name_id также вся таблица names в последствии будут удалены, пока оставлены для работы остальных приложений (AWeb и трекеров, потом будут переделаны), для обратной совместимости
* <b>1.90 - 04/02/22</b>
1. В Casher добавлены методы для сохранения локаций и комплектов устройств, дополнены соответственно методы в FireDBHelper. Старт приложения стал заметно быстрее. Уменьшилось количество обращений к БД
* <b>1.89 - 04/02/22</b>
